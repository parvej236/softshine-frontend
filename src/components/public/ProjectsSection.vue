<template>
  <section id="projects" class="py-24 lg:py-32 relative bg-[#020617] overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        <div class="max-w-2xl">
          <h2 class="text-5xl md:text-6xl font-black text-white tracking-tighter mb-4">
            Crafting Digital <br/>
        ...
        <div class="flex flex-wrap gap-2 p-1.5 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10">
          <button 
            ...
            :class="[
              'px-5 py-2.5 rounded-xl text-xs font-bold transition-all duration-500 uppercase tracking-tighter',
              activeCategory === cat 
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        <TransitionGroup name="project-card">
          <article
            ...
            :key="project.id || i"
            class="group relative bg-gradient-to-b from-white/10 to-transparent border border-white/10 rounded-[2.5rem] overflow-hidden transition-all duration-700 hover:border-cyan-500/50"
          >
            <div class="relative h-64 overflow-hidden m-3 rounded-[1.8rem]">
              <img :src="project.imageUrl" :alt="project.title"
                   class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
              
              <div class="absolute inset-0 bg-[#020617]/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center gap-4">
                <button @click="openLightbox(project)" class="p-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white hover:bg-cyan-500 hover:text-black transition-all">
                ...
                <a :href="project.projectLink" target="_blank" class="p-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white hover:bg-cyan-500 hover:text-black transition-all">
              ...
              <div class="absolute top-4 left-4">
                <span class="px-4 py-1.5 text-[10px] font-black tracking-widest bg-[#020617]/80 backdrop-blur-md text-cyan-400 border border-white/10 rounded-full uppercase">
                  {{ project.category }}
                </span>
              </div>
            </div>
            ...
            <div class="p-8 pt-4">
              <div class="flex justify-between items-start mb-4">
                <h3 class="text-2xl font-bold text-white tracking-tight group-hover:text-cyan-400 transition-colors">
                  {{ project.title }}
                </h3>
              </div>
              ...
              <div class="flex items-center gap-4">
                <a :href="project.projectLink" target="_blank"
                   class="flex-1 flex items-center justify-center gap-2 py-4 bg-white text-black text-xs font-black rounded-2xl hover:bg-cyan-400 transition-all active:scale-95">
                  LIVE DEMO
                </a>
                <a :href="project.githubLink" target="_blank"
                   class="w-14 h-14 flex items-center justify-center border border-white/10 rounded-2xl text-white hover:bg-white/5 transition-all">
      ...
      <div v-if="hasMore" class="mt-24 text-center">
        <button @click="visibleCount += 4" 
                class="inline-flex items-center gap-4 px-12 py-5 bg-transparent border border-white/10 text-white font-bold rounded-full hover:bg-white hover:text-black transition-all duration-500">
          EXPLORE MORE
...
const setCategory = (cat) => {
  ...
  visibleCount.value = 6 // Reset view count when changing categories
}
...
// Dummy data for visual perfection
const dummyProjects = Array.from({ length: 12 }).map((_, i) => ({
  ...
  projectLink: 'https://softshine.io', // Actual Public URL
