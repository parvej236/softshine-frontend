      ...
      <div class="grid lg:grid-cols-12 gap-12 items-start max-w-7xl mx-auto">
        ...
        <div class="lg:col-span-7">
          <form @submit.prevent="submitForm" class="relative p-8 md:p-12 rounded-[3rem] bg-white/[0.01] border border-white/10 backdrop-blur-3xl shadow-2xl space-y-6">
            <div v-if="successMessage" class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-sm font-semibold text-center">
              ✓ {{ successMessage }}
            </div>

            <div v-if="errorMessage" class="p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300 text-sm font-semibold text-center">
              ✕ {{ errorMessage }}
            </div>
            ...
            <button type="submit" :disabled="submitting" 
                    class="group relative w-full overflow-hidden px-8 py-5 bg-cyan-500 text-black font-black text-sm rounded-2xl transition-all duration-300 hover:shadow-[0_0_50px_rgba(6,182,212,0.4)] active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-50">
...
onMounted(() => {
  emailjs.init('fVATQAMJlu-Ilz_vM')
})
...
const submitForm = async () => {
  ...
  try {
    await emailjs.send(
      ...
      'template_s5617sw',
      {
        to_email: 'alamparvej2024@gmail.com',
